...
export default async function BatchPaymentPage() {
  ...
  const [{ data: parties }, { data: bankAccounts }, { data: pettyCashFunds }] = await Promise.all([
  ...
  return (
    <div className="space-y-4">
      ...
      <MultiPaymentForm parties={parties ?? []} bankAccounts={bankAccounts ?? []} pettyCashFunds={pettyCashFunds ?? []} />
    </div>
  );
}
