...
export function PodPanel({ dcId, canDispute }: { dcId: string; canDispute: boolean }) {
  ...
  function submitAccept() {
    ...
    if (!acceptedByName.trim()) {
      setError("Accept karne wale ka naam likhna zaroori hai.");
      return;
    }
  ...
  function submitDispute() {
    setError(null);
    if (!note.trim()) {
      setError("Dispute ki wajah likhna zaroori hai.");
      return;
    }
    startTransition(async () => {
    ...
    <div className="rounded-xl border border-line bg-surface p-4 space-y-3">
      ...
      {mode === "accept" && (
        <div className="space-y-2">
          <input
            ...
            placeholder="Accept karne wale ka naam"
      ...
      {mode === "dispute" && (
        <div className="space-y-2 rounded-md border border-bad bg-bad-soft p-3">
          <textarea value={note} onChange={(e) => setNote(e.target.value)} rows={2} placeholder="Dispute ki wajah…" className="input resize-none text-xs" />
          {error && <p className="text-xs text-bad">{error}</p>}
          <div className="flex gap-2">
            <button type="button" onClick={() => setMode("none")} className="flex-1 rounded-md border border-line-strong bg-bg px-2 py-1 text-xs">
              Wapis
