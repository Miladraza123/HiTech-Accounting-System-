...
import { cancelContraEntryAction } from "@/app/actions/cashBank";

export function CancelContraEntryButton({ transferId }: { transferId: string }) {
  ...
  function submit() {
    ...
    startTransition(async () => {
      const res = await cancelContraEntryAction(transferId, reason);
  ...
  return (
    <div className="space-y-2 rounded-md border border-bad bg-bad-soft p-3">
      <textarea value={reason} onChange={(e) => setReason(e.target.value)} rows={2} placeholder="Reason for cancellation…" className="input resize-none text-xs" />
